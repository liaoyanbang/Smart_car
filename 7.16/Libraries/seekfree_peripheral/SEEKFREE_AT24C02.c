...
void at24c02_write_uint16(uint8 byte_reg, uint16 dat)
{
	delay_ms(6);
	simiic_write_reg(AT24C02_DEV_ADDR, byte_reg, dat>>8);
	delay_ms(6);
	simiic_write_reg(AT24C02_DEV_ADDR, byte_reg+1,dat);
}
...
uint16 at24c02_read_uint16(uint8 byte_reg)
{
	uint16 val = 0;
	delay_ms(6);
	val = simiic_read_reg(AT24C02_DEV_ADDR, byte_reg, SIMIIC);
	val = val<<8 | at24c02_read_byte(byte_reg+1);
	return val;
}
//-------------------------------------------------------------------------------------------------------------------
//  @brief      at24c02写多个字节
