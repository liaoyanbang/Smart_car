...
#define uart_data_MAXLEN  5


uint8 uart_data[uart_data_MAXLEN];
uint16 uart_data_len = 0;
uint8 uart_flag = 0;
char res ;
...
void bluetooth_ch9141_uart_callback(void)
{
    ...
    else if(0 == at_mode)
    {
        ...
        //透传模式 用户在此处接收配对的蓝牙发送过来的数据
        //接到一个字节后单片机将会进入此处，通过在此处读取bluetooth_ch9141_rx_buffer可以取走数据
				//if(BLUETOOTH_CH9141_DATA_BUF != '!' && uart_flag==0)
//				if(uart_flag ==0)
//				{
						...
						//uart_flag = 1;
			//	}
//				else if(BLUETOOTH_CH9141_DATA_BUF=='!' && uart_flag==0)
//				{
//						uart_flag = 1;
//						i=0;
//				}
